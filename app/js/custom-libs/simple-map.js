...
document.addEventListener("DOMContentLoaded", function () {
  ...
  mapElements.forEach(mapElement => {
    initSimpleMap(mapElement);
    initSimpleMapOverlay(mapElement);
  });

  async function initSimpleMap(mapItem) {
    await ymaps3.ready;
    ...
    // Дефолтный маркер myMap.addChild(new YMapDefaultMarker(marker))
    const {YMapDefaultMarker} = await ymaps3.import('@yandex/ymaps3-markers@0.0.1');

    let mapCoords = mapItem.getAttribute("data-map-coords").split(',').map(cord => +cord)
    let mapIcon = mapItem.getAttribute("data-map-icon");
    let mapIconCaption = mapItem.getAttribute("data-map-icon-caption");
    let mapBalloon = mapItem.getAttribute("data-map-balloon");
    //  инициализация карты
    let myMap = new YMap(mapItem, {
      location: {
        center: mapCoords,
        zoom: 17
      }
    ...
    const marker= {
      ...
      title: mapIconCaption,
      bolloon: mapBalloon,
      caption: mapIconCaption,
      icon: mapIcon,
      // подключение кастомного маркера
      element: AddPlacemark,
    }

    function AddPlacemark(props) {
      const placemark = document.createElement('div');
      placemark.classList.add('placemark')
      //  Добавления иконки
      placemark.insertAdjacentHTML('beforeend',
        `<img src=${props.icon}>`)

      // добавления bolloon
      if(props.bolloon) {
        placemark.insertAdjacentHTML('beforeend',
          `<div class="placemark__caption">${mapBalloon}</div`)
      }
      placemark.title = props.title;
      return placemark;
    }

    myMap.addChild(new YMapDefaultFeaturesLayer());
    // Добавления маркера
    myMap.addChild(new YMapMarker(marker, marker.element(marker)))
